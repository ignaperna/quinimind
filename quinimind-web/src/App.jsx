 ...
 * --------------------------
 * Since we only have the latest result from the web scrape, 
 * we will simulate historical data to make the 'SaaS' features (statistics) functional.
 */

// Real data scraped from the browsing tool
const LATEST_DRAW = {
  id: 3330,
  date: "14/12/2025",
  modes: {
    tradicional: [0, 25, 26, 28, 34, 41],
    laSegunda: [8, 17, 28, 29, 35, 43],
    revancha: [4, 6, 16, 22, 34, 36],
    siempreSale: [2, 13, 31, 32, 40, 43]
  }
};

// Generate mock history for statistics
...
const MOCK_HISTORY = generateHistory(50); // Last 50 draws for stats
...
const ResultsSection = () => {
  const modes = [
    { name: "Tradicional", nums: LATEST_DRAW.modes.tradicional, color: "text-blue-400" },
    { name: "La Segunda", nums: LATEST_DRAW.modes.laSegunda, color: "text-emerald-400" },
    { name: "Revancha", nums: LATEST_DRAW.modes.revancha, color: "text-amber-400" },
    { name: "Siempre Sale", nums: LATEST_DRAW.modes.siempreSale, color: "text-rose-400" },
  ];
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Sorteo #{LATEST_DRAW.id} • {LATEST_DRAW.date}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {modes.map((mode) => (
          <Card key={mode.name} className="relative overflow-hidden group hover:border-slate-600 transition-colors">
            ...
            <h3 className={`font-semibold mb-4 ${mode.mode} text-slate-200`}>{mode.name}</h3>
...
const StatsSection = ({ history }) => {
  // Calculate frequency
  const stats = useMemo(() => {
    const frequency = Array(46).fill(0);
    // Add current draw
    Object.values(LATEST_DRAW.modes).flat().forEach(n => frequency[n]++);
    // Add history
    ...
    <div className="space-y-6 mt-12">
      <div className="flex items-center justify-between">
        ...
        <select className="bg-slate-800 border-slate-700 text-slate-300 rounded-lg text-sm px-3 py-1">
          ...
          <option>Últimos 100 Sorteos</option>
          <option>Histórico 2024</option>
        </select>
      </div>
...
const PredictionEngine = ({ isPro, onUpgrade }) => {
  ...
  const generatePrediction = () => {
    ...
    setTimeout(() => {
      ...
      setPrediction(nums.sort((a, b) => a - b));
      setLoading(false);
    }, 1500);
  };
      ...
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-r from-violet-900 to-indigo-900 border border-violet-700/50 shadow-2xl p-8 lg:p-12">
        ...
        <div className="relative z-10 text-center max-w-2xl mx-auto">
          ...
          <p className="text-violet-200 mb-8 text-lg">
            Nuestros algoritmos analizan miles de sorteos históricos para detectar patrones de probabilidad y sugerir combinaciones optimizadas.
          </p>
            ...
            <div className="space-y-8">
              ...
              {prediction && (
                <p className="text-sm text-violet-300/70">
                  Probabilidad estimada de acierto: <span className="text-white font-bold">Calculando...</span>
                </p>
              )}
            </div>
          )}
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-violet-500/30">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-16">
        ...
        {/* STATS DASHBOARD */}
        <StatsSection history={MOCK_HISTORY} />
